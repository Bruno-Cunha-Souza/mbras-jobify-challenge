...
const ToogleTheme: React.FC = () => {
  ...
  const handleToggle = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newTheme = isChecked ? "🌑" : "☀️";
    setLocalTheme(newTheme);
    toggleTheme(newTheme);
  };

  return (
    <StyledToogleTheme className={theme === "🌑" ? "dark" : "light"}>
      <label className={`${"theme-switch"}`}>
        <input type="checkbox" className="theme-switch__checkbox" onChange={handleToggle} checked={localTheme === "🌑"} />
...
const StyledToogleTheme = styled.div`
  ...
  .theme-switch__clouds {
    ...
    bottom: -0.625em;
    left: 0.312em;
    box-shadow: 0.937em 0.312em var(--clouds-color), -0.312em -0.312em var(--back-clouds-color), 1.437em 0.375em var(--clouds-color),
  ...
  &.dark .theme-switch__container {
    background-color: var(--container-night-bg);
  }

  &.dark .theme-switch__circle-container {
    left: calc(100% - var(--circle-container-offset) - var(--circle-container-diameter));
  }
  ...
  &.dark .theme-switch__clouds {
    bottom: -4.062em;
  }

  &.dark .theme-switch__stars-container {
    top: 50%;
    transform: translateY(-50%);
  }
`;
